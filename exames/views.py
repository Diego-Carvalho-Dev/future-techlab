...
def permitir_abrir_exame(request, exame_id):
    ...
    if not exame.requer_senha:
        if not exame.resultado:
            messages.add_message(request, constants.ERROR, "Ainda não foi cadastrado seu resultado, por favor entre em contato!")
...
def acesso_medico(request, token):
    ...
    pedidos = PedidoExames.objects.filter(data__gte = acesso_medico.data_exames_iniciais).filter(data__lte = acesso_medico.data_exames_finais).filter(usuario=acesso_medico.usuario)
